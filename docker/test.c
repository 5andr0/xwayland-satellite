...
{
    const char *value = getenv("TEST_USE_TITLEBAR");
    char *end = NULL;
    long parsed;
...
int main(void)
{
    ...
    XSelectInput(display,
                 app.window,
                 ExposureMask | KeyPressMask | StructureNotifyMask | ButtonPressMask | ButtonReleaseMask | ButtonMotionMask);
    ...
    for (;;) {
        if (!requested_position) {
            ...
            XFlush(display);
            fprintf(stdout,
                    "raw test requested position %d,%d size=%dx%d\n",
                    target.x,
                    target.y,
                    width,
                    height);
