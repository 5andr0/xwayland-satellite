...
case "${BRANCH}" in
    ""|main)
        ;;
    start_pos_fix|dragfix|noscale|giga_patch)
        SATELLITE_BIN="/opt/xwayland-satellite/bin/${BRANCH}/xwayland-satellite"
        ;;
    xwayland|host)
        ;;
    *)
...
if [ "${BRANCH}" = "xwayland" ] || [ "${BRANCH}" = "host" ]; then
...
if [ "${BRANCH}" = "noscale" ]; then
    "${SATELLITE_BIN}" ":${DISPLAY_NUM}" -noscaling &
else
